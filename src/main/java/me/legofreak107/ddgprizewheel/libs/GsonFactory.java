...
public class GsonFactory {
   ...
   */
    ...
    public static Gson getNewGson(boolean prettyPrinting) {
        if(newGson == null) {
            GsonBuilder newGsonBuilder = new GsonBuilder().addSerializationExclusionStrategy(new ExposeExlusion())
                    .addDeserializationExclusionStrategy(new ExposeExlusion())
    ...
    public static class NewItemStackAdapter extends TypeAdapter<ItemStack> {
        ...
        public void write(JsonWriter jsonWriter, ItemStack itemStack) throws IOException {
            ...
            net.minecraft.server.v1_12_R1.ItemStack item = removeSlot(itemStack);
            if (item == null) {
                jsonWriter.nullValue();
                return;
            }
            try {
                ...
                if (item != null && item.getTag() != null) {
                    jsonWriter.value(writeValue(item.getTag()));
                } else
                    jsonWriter.value("");
    ...
    public static String writeValue(NBTTagCompound compound) {
        ...
        return new StringBuilder(PREFIX).append(SPLIT).append(nbtToSTBase64(compound)).toString();
    }
    public static NBTTagCompound readValue(String raw) {
        if(raw.equalsIgnoreCase("")) return null;
        try {
            String[] str = raw.split(SPLIT);
